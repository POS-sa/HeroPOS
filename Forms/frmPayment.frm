...
Begin VB.Form frmPayment 
   ...
   Begin VSFlex8Ctl.VSFlexGrid grdPay 
      ...
      FormatString    =   $"frmPayment.frx":000C
      ScrollTrack     =   -1  'True
      ScrollBars      =   0
      ScrollTips      =   0   'False
      MergeCells      =   5
...
Private Sub ButtonEx1_Click()
    ActiveReadServer1 "Select isnull(max(Payment_No),0)+1 as Receipt_No from Supplier_Accounts where Transaction_Type = 'Payment'"
    ...
    Tender_Type = ""
    Ref_No = ""
    dd = frmPayment.txtGrvNo
    ActiveReadServer "Select Balance from Supplier_Accounts where Account_No = '" & frmPayment.txtGrvNo & "' order by Line_No"
    ...
    If Tender_Type = "" Then
        ...
        Exit Sub
    End If
    If frmPayment.Tag = "Account" Then
        ...
        "VALUES(" & UserRecord.User_Number & ",Getdate(),'Payment','" & txtInvoice.Text & "','" & txtGrvNo.Text & "'," & Payment & ",0," & Balance - (Payment) & ",'" & Tender_Type & "','" & Ref_No & "'," & Payment_No & ")"
        DoEvents
        ActiveUpdateServer "Update Suppliers set Balance= Balance - " & Payment & " where Supplier_No='" & txtGrvNo.Text & "'"
    Else
        ActiveUpdateServer "INSERT INTO [Supplier_Accounts]([User_No],[Date_Time],[Transaction_Type], [Invoice_No], [Account_No], [Debit], [Credit], [Balance],[Tender_Type],[Ref_No],[Payment_No])" & _
        "VALUES(" & UserRecord.User_Number & ",Getdate(),'Payment','" & txtInvoice.Text & "','" & Trim(Replace(Mid(txtDescription.Text, InStrRev(txtDescription.Text, "(") + 1), ")", "")) & "'," & Payment & ",0," & Balance - (Payment) & ",'" & Tender_Type & "','" & Ref_No & "'," & Payment_No & ")"
        DoEvents
        ActiveUpdateServer "Update Suppliers set Balance= Balance - " & Payment & " where Supplier_No='" & Trim(Replace(Mid(txtDescription.Text, InStrRev(txtDescription.Text, "(") + 1), ")", "")) & "'"
    End If
    Unload Me
End Sub
...
Private Sub Form_Activate()
    ...
    If frmPayment.Tag = "Account" Then
        ...
        txtTotal.Text = frmAccount.lblTotal.Caption
        Label1.Caption = "Account No: "
        Label3.Caption = "Balance Due: "
        txtInvoice.Text = frmAccount.grdAcc.TextMatrix(frmAccount.grdAcc.Row, 5)
    Else
        txtInvoice.Text = frmGRV.txtInvoice.Text
        txtGrvNo.Text = frmGRV.lblGRV.Caption
        txtDescription.Text = frmGRV.cmbSuppliers.Text
        txtTotal.Text = frmGRV.txtTotal.Text
        Label1.Caption = "GRV No: "
        Label3.Caption = "Invoice Total: "
    End If
End Sub
Private Sub grdPay_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    If grdPay.Col = 1 Then
        If grdPay.ValueMatrix(grdPay.Row, grdPay.Col) <> Abs(Val(txtTotal.Text)) Then
