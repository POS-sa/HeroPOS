...
Begin VB.Form frmSearch 
   ...
   Begin VSFlex8Ctl.VSFlexGrid grdProd 
      ...
      FormatString    =   $"frmSearch.frx":0022
      ScrollTrack     =   -1  'True
      ScrollBars      =   2
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin btButtonEx.ButtonEx cmdOk 
      ...
      EndProperty
   End
   Begin btButtonEx.ButtonEx cmdFind 
      Height          =   375
      Left            =   2400
      TabIndex        =   4
      Top             =   7380
...
Private Sub cmdFind_Click()
    ...
    Select Case frmSearch.Tag
        ...
        Case "Order"
            Location = Location_No
        Case ""
            Location = Val(Mid(frmGRV.grdGRV.TextMatrix(frmGRV.grdGRV.Row, 2), 1, InStr(frmGRV.grdGRV.TextMatrix(frmGRV.grdGRV.Row, 2), "-") - 1))
            If frmGRV.cmdProducts.Tag = "C" Then
                frmGRV.cmdProducts.Tag = ""
                OldLocation = 0
            End If
    ...
    If frmSearch.Tag = "Specials" Then
        ...
        "+ Unit_of_Measure END AS Description from Products where Sales_Item = 1"
    Else
        ...
        Select Case rs.Fields("Loc_Type")
            Case 0, 1
                Select Case frmSearch.Tag
                    Case "Links"
                        adoData.RecordSource = "Select Product_Code,  CASE Unit_Size WHEN 0 THEN Products.Description + ' ' + Unit_of_Measure ELSE Products.Description + ' ' + CONVERT(nvarchar(20), Unit_Size) " & _
                        "+ Unit_of_Measure END AS Description from Products where Pack_Size =1 and Stock_Item=1 and Sales_Item = 1 and Department_No in" & _
                        " (Select Department_No from Departments_Stock where Location_No = " & Location & ") and Description like '%" & txtFind.Text & "%' order by Description"
                    Case "Appros"
                        adoData.RecordSource = "Select Product_Code,  CASE Unit_Size WHEN 0 THEN Products.Description + ' ' + Unit_of_Measure ELSE Products.Description + ' ' + CONVERT(nvarchar(20), Unit_Size) " & _
                        "+ Unit_of_Measure END AS Description from Products where Sales_Item = 1 and Department_No in" & _
                        " (Select Department_No from Departments_Stock where Location_No = " & Location & ") and Description like '%" & txtFind.Text & "%' order by Description"
                    Case Else
                        adoData.RecordSource = "Select Product_Code,  CASE Unit_Size WHEN 0 THEN Products.Description + ' ' + Unit_of_Measure ELSE Products.Description + ' ' + CONVERT(nvarchar(20), Unit_Size) " & _
                        "+ Unit_of_Measure END AS Description from Products where Returnable_Item =1 or (Stock_Item = 1 and Department_No in" & _
                        " (Select Department_No from Departments_Stock where Location_No = " & Location & "))  and Description like '%" & txtFind.Text & "%' order by Description"
                End Select
            Case 2
                adoData.RecordSource = "Select Product_Code,  CASE Unit_Size WHEN 0 THEN Products.Description + ' ' + Unit_of_Measure ELSE Products.Description + ' ' + CONVERT(nvarchar(20), Unit_Size) " & _
                "+ Unit_of_Measure END AS Description from Products where Stock_Item = 1 and Department_No in" & _
                " (Select Department_No from Departments_Stock where Location_No = " & Location & ")  or (Stock_Item = 0 and Sales_Item = 0)  and Description like '%" & txtFind.Text & "%' order by Description"
        End Select
    End If
    adoData.Refresh
    grdProd.SetFocus
    grdProd.Col = 1
    txtFind.Text = "<Type to Search>"
...
Private Sub cmdOk_Click()
    frmSearch.Tag = grdProd.TextMatrix(grdProd.Row, 1) & " - " & grdProd.TextMatrix(grdProd.Row, 0)
    Me.Hide
End Sub
...
Private Sub txtFind_GotFocus()
    If txtFind.Text = "<Type to Search>" Then
        txtFind.Text = ""
        txtFind.Tag = "C"
    End If
End Sub
