VERSION 5.00
Begin {78E93846-85FD-11D0-8487-00A0C90DC8A9} rptCountSheetnormbarcoded 
   Caption         =   "Stock Count Sheet"
   ClientHeight    =   11010
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   11880
   ...
   BeginProperty Section0 {1C13A8E0-A0B6-11D0-848E-00A0C90DC8A9} 
      ...
      BeginProperty Item0 {1C13A8E5-A0B6-11D0-848E-00A0C90DC8A9} 
         ...
         Object.Width           =   1575
         Object.Height          =   1875
         Picture         =   "rptCountSheetnormbarcoded.dsx":0000
         SizeMode        =   1
      EndProperty
      ItemType1       =   6
   ...
   BeginProperty Section1 {1C13A8E0-A0B6-11D0-848E-00A0C90DC8A9} 
      ...
      BeginProperty Item2 {1C13A8E1-A0B6-11D0-848E-00A0C90DC8A9} 
         ...
         EndProperty
         ForeColor       =   8421504
         Object.Caption         =   "STOCK COUNT SHEET"
      EndProperty
      ItemType3       =   3
      BeginProperty Item3 {1C13A8E1-A0B6-11D0-848E-00A0C90DC8A9} 
         _Version        =   393216
   ...
   EndProperty
End
Attribute VB_Name = "rptCountSheetnormbarcoded"
...
Private Sub DataReport_Initialize()
    ...
    Select Case .lblDepart.Tag
        Case ""
            Location_No = Mid(.grdLoc.TextMatrix(.grdLoc.Row, 0), 1, InStr(.grdLoc.TextMatrix(.grdLoc.Row, 0), "-") - 1)
            Department_No = ""
        Case Else
            Location_No = Mid(.grdLoc.TextMatrix(.grdLoc.Row, 0), 1, InStr(.grdLoc.TextMatrix(.grdLoc.Row, 0), "-") - 1)
            Department_No = .cmdDeptStrip(Val(.lblDepart.Tag)).Tag
    End Select
    End With
    Select Case Department_No
        Case ""
            sSQL = "Select '*' + Product_Code + '*' as Product_Code,Department_No,(Select Dept_Name from Departments where Departments.Department_No = Products.Department_No) as Dept_Name, CASE Unit_Size WHEN 0 THEN Products.Description + ' ' + Unit_of_Measure ELSE Products.Description + ' ' + CONVERT(nvarchar(20), Unit_Size) " & _
            "+ Unit_of_Measure END AS Description, Unit_Size, Unit_of_Measure from Products where Stock_Item = 1 and Department_No in" & _
    ...
    Barcodes = Replace(MyRecSet.Fields("Product_Code"), "*", "")
    
    
    
    
    
    Dim MyConn2 As ADODB.Connection
    Dim MyRecSet2 As ADODB.Recordset
    Dim sSQL2 As String
 
    Set MyConn2 = New ADODB.Connection
    Set MyRecSet2 = New ADODB.Recordset
    MyConn2.ConnectionString = cnnMain.ConnectionString
    MyConn2.Open
    
    
         sSQL2 = "Select *  from Barclinks where Product_Code = '" & Barcodes & "'"
         ',  Product_Code as Products_Code,BarCode_Links as BarCode_Links"

        
    
    
    
    'Products_Code , BarCode_Links
    
    
    
    File = MyRecSet2.Fields(BarCode_Links)
    
    
   ' File = (App.Path & "\Barcodes\" & Barcodes & "." & "_")
    Set rptCountSheetnormbarcoded.DataSource = MyRecSet
    On Error Resume Next
    Set rptCountSheetnormbarcoded.Sections(1).Controls(1).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
    
    rptCountSheetnormbarcoded.Sections(1).Controls(3).Caption = Branch_Name
    rptCountSheetnormbarcoded.Sections(1).Controls(4).Caption = Branch_Address
    
    If Barcodes <> "" Then Set rptCountSheetnormbarcoded.Sections(3).Controls(6).Picture = LoadPicture(File)
    If Barcodes = "" Then Set rptCountSheetnormbarcoded.Sections(3).Controls(6).Picture = LoadPicture(File)
