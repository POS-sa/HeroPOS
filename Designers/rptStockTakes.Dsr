VERSION 5.00
Begin {78E93846-85FD-11D0-8487-00A0C90DC8A9} rptStockTakes 
   Caption         =   "Stock Takes..."
   ClientHeight    =   11010
   ClientLeft      =   60
   ClientTop       =   450
   ...
   BeginProperty Section0 {1C13A8E0-A0B6-11D0-848E-00A0C90DC8A9} 
      ...
      BeginProperty Item0 {1C13A8E5-A0B6-11D0-848E-00A0C90DC8A9} 
         ...
         Object.Width           =   1575
         Object.Height          =   1875
         Picture         =   "rptStockTakes.dsx":0000
         SizeMode        =   1
      EndProperty
      ItemType1       =   6
...
Private Sub DataReport_Initialize()
    ...
    With frmReports
        If Right(Str(Time_Stop), 2) = "AM" And .mthViewStart.Value = .mthViewEnd.Value Then
            .mthViewEnd.Value = DateAdd("d", 1, .mthViewEnd.Value)
        Else
            Selender = .mthViewEnd.Value
        End If
        If frmReports.cmb1.Text = "<All Locations>" Then
            sSQL = "Select * from Stock_Take_View where " & _
        ...
        Else
            sSQL = "Select * from Stock_Take_View where Location_No=" & Mid(frmReports.cmb1.Text, 1, InStr(frmReports.cmb1.Text, "-") - 1) & _
    ...
    End With
    Set rptStockTakes.DataSource = MyRecSet
    On Error Resume Next
    Set rptStockTakes.Sections(1).Controls(1).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
    rptStockTakes.Sections(1).Controls(3).Caption = Branch_Name
    rptStockTakes.Sections(1).Controls(4).Caption = Branch_Address
    rptStockTakes.Sections(2).Controls(10).Caption = "Reporting from " & frmReports.lblDate.Caption
    On Error Resume Next
    Set rptStockTakes.Sections(1).Controls(1).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
'    On Error Resume Next
'    For i = 1 To rptStockTakes.Sections(2).Controls.Count
'        rptStockTakes.Sections(2).Controls(i).Caption = Str(i)
'    Next i
'    On Error GoTo 0
    Exit Sub
