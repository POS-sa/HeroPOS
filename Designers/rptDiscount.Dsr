VERSION 5.00
Begin {78E93846-85FD-11D0-8487-00A0C90DC8A9} rptDiscount 
   Caption         =   "DataReport1"
   ClientHeight    =   11010
   ClientLeft      =   60
   ClientTop       =   450
...
Private Sub DataReport_Activate()
    ...
    With frmReports
        ...
        If .cmb1.Text = "<All Users>" Then
            sSQL = "Select * from Discount_View where " & _
        ...
        Else
            sSQL = "Select * from Discount_View where User_No = " & Val(Mid(.cmb1.Text, 1, InStr(.cmb1.Text, "-") - 1)) & " and " & _
            " (Date_Time > '" & .mthViewStart.Value & " " & Format(Time_Start, "hh:mm:ss AM/PM") & "' and Date_Time<'" & Selender & " " & Format(Time_Stop, "hh:mm:ss AM/PM") & "') order by Date_Time"
        End If
    End With
    MyRecSet.Open sSQL, MyConn, adOpenStatic, adLockReadOnly

    Set rptDiscount.DataSource = MyRecSet
    On Error Resume Next
    Set rptDiscount.Sections(1).Controls(1).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
    rptDiscount.Sections(1).Controls(3).Caption = Branch_Name
    rptDiscount.Sections(1).Controls(4).Caption = Branch_Address
    rptDiscount.Sections(2).Controls(18).Caption = "Reporting from " & frmReports.lblDate.Caption
    On Error Resume Next
    Set rptDiscount.Sections(1).Controls(1).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
End Sub
