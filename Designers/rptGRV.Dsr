VERSION 5.00
Begin {78E93846-85FD-11D0-8487-00A0C90DC8A9} rptGRV 
   ...
   BeginProperty Section1 {1C13A8E0-A0B6-11D0-848E-00A0C90DC8A9} 
      ...
      BeginProperty Item2 {1C13A8E1-A0B6-11D0-848E-00A0C90DC8A9} 
         ...
         EndProperty
         ForeColor       =   8421504
         Object.Caption         =   "GOODS RECEIVED VOUCHER"
      EndProperty
      ItemType3       =   3
   ...
   BeginProperty Section4 {1C13A8E0-A0B6-11D0-848E-00A0C90DC8A9} 
      ...
      BeginProperty Item19 {1C13A8E1-A0B6-11D0-848E-00A0C90DC8A9} 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Object.Caption         =   "Payment Value:"
         Alignment       =   1
      EndProperty
   EndProperty
End
Attribute VB_Name = "rptGRV"
...
Private Sub DataReport_Activate()
    ...
    MyConn.Open
    If frmMain.cmdBar(5).Enabled = False Then
        sSQL = "SELECT * from GRV_Printout where GRV_No='" & frmReports.grdMain.TextMatrix(frmReports.grdMain.Row, 8) & "' and Supplier_No is not Null"
    Else
        sSQL = "SELECT * from GRV_Printout where GRV_No=" & Val(frmMain.Toolbar1.Tag) & " and Supplier_No is not Null"
    End If
    MyRecSet.Open sSQL, MyConn, adOpenStatic, adLockReadOnly

    Set rptGRV.DataSource = MyRecSet
    
'    On Error Resume Next
'    For i = 1 To rptGRV.Sections(5).Controls.Count
'        rptGRV.Sections(5).Controls(i).Caption = Str(i)
'    Next i
'    On Error GoTo 0
    On Error Resume Next
    Set rptGRV.Sections(1).Controls(6).Picture = LoadPicture(Logo_File)
    On Error GoTo 0
    rptGRV.Sections(1).Controls(8).Caption = Branch_Name
    
    rptGRV.Sections(1).Controls(9).Caption = Branch_Address
    
    rptGRV.Sections(1).Controls(13).Caption = Trim(MyRecSet.Fields("Supplier_No")) & " - " & MyRecSet.Fields("Supplier_Name")
    Supplier_No = Trim(MyRecSet.Fields("Supplier_No"))
    rptGRV.Sections(1).Controls(14).Caption = MyRecSet.Fields("Address") & ""
    rptGRV.Sections(1).Controls(17).Caption = Format(MyRecSet.Fields("Order_No"), "00000")
    rptGRV.Sections(1).Controls(18).Caption = MyRecSet.Fields("Vat_No") & ""
    rptGRV.Sections(1).Controls(19).Caption = MyRecSet.Fields("Contact_Person") & ""
    rptGRV.Sections(1).Controls(20).Caption = MyRecSet.Fields("Invoice_No") & ""
    
    rptGRV.Sections(1).Controls(29).Caption = Format(MyRecSet.Fields("Date_Time"), "ddd dd MMM yyyy")
    rptGRV.Sections(1).Controls(30).Caption = Format(MyRecSet.Fields("Order_Date"), "ddd dd MMM yyyy")
    rptGRV.Sections(1).Controls(32).Caption = Format(MyRecSet.Fields("Delivery_Date"), "ddd dd MMM yyyy")
    rptGRV.Sections(1).Controls(31).Caption = Format(MyRecSet.Fields("Invoice_Date"), "ddd dd MMM yyyy")
    
    rptGRV.Sections(2).Controls(21).Caption = "GRV No: " & Format(MyRecSet.Fields("GRV_No"), "000000")
    Total = 0
    Subtotal = 0
    While Not MyRecSet.EOF
        Total = Total + MyRecSet.Fields("Line_Total") * ((100 + MyRecSet.Fields("Vat_Rate")) / 100)
        Subtotal = Subtotal + MyRecSet.Fields("Line_Total")
        MyRecSet.MoveNext
    Wend
    MyRecSet.MoveFirst
    rptGRV.Sections(5).Controls(5).Caption = Format(Subtotal, "0.00")
    rptGRV.Sections(5).Controls(6).Caption = Format(Total - Subtotal, "0.00")
    rptGRV.Sections(5).Controls(7).Caption = Format(Total, "0.00")
    frmMain.Toolbar1.Buttons(11).Enabled = True
    frmMain.Toolbar1.Buttons(12).Enabled = True
    
    ActiveReadServer "Select * from Supplier_Accounts where Transaction_Type = 'Payment' and Account_No = '" & Supplier_No & "' and Invoice_No = '" & rptGRV.Sections(1).Controls(20).Caption & "'"
    If rs.RecordCount > 0 Then
        rptGRV.Sections(5).Controls(15).Caption = Format(rs.Fields("Date_Time"), "DDD DD MMM YYYY")
        rptGRV.Sections(5).Controls(16).Caption = rs.Fields("Tender_Type") & " - Ref: " & rs.Fields("Ref_No")
        rptGRV.Sections(5).Controls(17).Caption = Format(rs.Fields("Debit"), "0.00")
    Else
        rptGRV.Sections(5).Controls(15).Caption = "Not Paid"
        rptGRV.Sections(5).Controls(16).Caption = ""
        rptGRV.Sections(5).Controls(17).Caption = ""
    End If
    rs.Close
End Sub
